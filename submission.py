...
class TimeValueAgent:
    ...
    def run(self, obs, config):
        ...
        genval = 1.0
        for i in range(40):
            self.ship_move_cost_ratios.append(genval)
        ...
        try:
            ...
            for plan in plans:
                if len(plan) > 1:
                    if plan[0] != plan[1]:
                        shipUid = self.board.ships[plan[0]]["uid"]
                        if plan[1] == -1:
                            action[shipUid] = "CONVERT"
                        else:
                            direction = self.get_direction(plan[0], plan[1])
                            action[shipUid] = direction
                    if plan[1] != -1:
                        updated_ships.append(plan[1])

            ship_count = len(self.ships)
            remaining_halite = sum(self.obs.halite)
    ...
    def get_col_row(self, pos):
        """Gets the column and row index of a position as a single tuple."""
        return (self.get_col(pos), self.get_row(pos))
    ...
    def manhattan_distance(self, pos1, pos2):
        ...
        dx = self.manhattan_distance_single(self.get_col(pos1), self.get_col(pos2))
        dy = self.manhattan_distance_single(self.get_row(pos1), self.get_row(pos2))
        return dx + dy

    def manhattan_distance_single(self, i1, i2):
        """Gets the distance in one dimension between two columns or two rows, including wraparound."""
        iMin = min(i1, i2)
        iMax = max(i1, i2)
        return min(iMax - iMin, iMin + self.size - iMax)
    ...
    def get_new_pos(self, pos, direction):
        ...
        col, row = self.get_col_row(pos)
        if direction == "NORTH":
            return (
                pos - self.size
                if pos >= self.size
                else self.size**2 - self.size + col
            )
        elif direction == "SOUTH":
            return col if pos + self.size >= self.size**2 else pos + self.size
        elif direction == "EAST":
            return pos + 1 if col < self.size - 1 else row * self.size
        elif direction == "WEST":
            return pos - 1 if col > 0 else (row + 1) * self.size - 1

    def get_neighbors(self, pos):
        """Returns the possible destination positions from the given one, in the order N/S/E/W."""
        neighbors = []
        col, row = self.get_col_row(pos)
        neighbors.append(self.get_new_pos(pos, "NORTH"))
        neighbors.append(self.get_new_pos(pos, "SOUTH"))
        neighbors.append(self.get_new_pos(pos, "EAST"))
        neighbors.append(self.get_new_pos(pos, "WEST"))
        return neighbors
    ...
    def get_direction(self, from_pos, to_pos):
        ...
        neighbors = self.get_neighbors(from_pos)
        if to_pos == neighbors[0]:
            ...
            raise Exception(
                "Could not determine direction from "
                + str(from_pos)
                + " to "
                + str(to_pos)
            )

    def get_current_value(self, path, plans, starting_halite):
        ...
        if self.is_blocked(path, plans):
            return -1

        for i in range(1, len(path)):
            if path[i] == path[i - 1]:
                if path[i] in self.shipyards:
                    # Dropoff - short-circuit, only lose time-value up to now
                    total_value_loss = self.time_value_ratios[i]
                    delivered_halite += final_halite * total_value_loss
                    final_halite = 0.0
                else:
                    mined_pos = path[i]
                    mined_halite = self.obs.halite[path[i]] * 0.25
                ...
                final_halite = (
                    final_halite - self.config.moveCost - self.config.convertCost
                )
                total_value_loss = self.time_value_ratios[i]
                delivered_halite += final_halite * total_value_loss
                final_halite = 0.0
            else:
                final_halite = final_halite - final_halite * self.config.moveCost
        ...
        total_movement_value_loss = self.ship_move_cost_ratios[remaining_steps]

        # In a sense, we can also say we are one more time-step away from a further harvest at this location.
        potential_harvest = (
            self.obs.halite[path[-1]] * 0.25 * self.time_value_ratios[len(path) + 1]
        )
        ...
        current_adjusted_value = (
            final_halite + potential_harvest
        ) * total_movement_value_loss * total_time_value_loss + delivered_halite
        ...
        potential_dropoff_value = (
            final_halite
            - self.config.spawnCost
            - self.config.convertCost
            + delivered_halite
        )

        return max(current_adjusted_value, potential_dropoff_value)

    def distance_to_closest_dropoff(self, pos):
        """Determines the manhattan distance (number of moves) from the given position
        to the nearest shipyard."""
        min_distance = self.size

        for d in self.shipyards:
            if d == pos:
                return 0
            distance = self.manhattan_distance(pos, d)
    ...
    def get_best_move(
        ...
        if path_so_far is None:
            ...
            if self.is_blocked(path_so_far, plans):
                return None
        ...
        for next_pos in next_pos_choices:
            ...
            best_move = self.get_best_move(
                next_pos, plans, starting_halite, path_so_far, max_depth - 1
            )
        ...
        for choice in choices:
            value = self.get_current_value(choice, plans, starting_halite)
            if value > best_value:
                best_value = value
                best_choice = choice
    ...
    def make_plans(self):
        ...
        for shipData in self.ships.values():
        ...
        for i in reversed(range(len(unplanned))):
            if (
                unplanned[i] in self.shipyards
                and self.board.ships[unplanned[i]]["halite"] > 0
            ):
        ...
        while len(unplanned) > 0:
            ship = unplanned.pop()
            best_ship_path = self.get_best_move(
            ...
            if self.debug:
                print(
                    ...
                    self.get_current_value(
                        best_ship_path, plans, self.board.ships[ship]["halite"]
                    ),
                )
            ...
            if self.is_blocked(best_ship_path, plans):
                for plan_index in range(len(plans)):
                    if (
                        len(plans[plan_index]) > 1
                        and plans[plan_index][1] == best_ship_path[1]
                    ):
    ...
    def is_blocked(self, path, plans):
        ...
        for plan in plans:
            if len(plan) >= len(path):
                if plan[len(path) - 1] == path[-1]:
                    ...
                    len(path) > 1
                    and plan[len(path) - 2] == path[-1]
                    and plan[len(path) - 1] == path[-2]
                ):
                    return True
        return False
        ...
        for index, player in enumerate(obs.players):
            ...
            for uid, ship in ships.items():
                ...
                for direction in ["NORTH", "EAST", "SOUTH", "WEST"]:
                    self.possible_ships[Board.get_to_pos(size, pos, direction)][
                        uid
                    ] = details
