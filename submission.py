...
def get_time_value_agent(
    ...
    def time_value_agent(obs, config):
        ...
        def get_current_value(path, plans, starting_halite):
            ...
            potential_harvest = (
                obs.halite[path[-1]] * 0.25 * time_value_ratios[len(path) + 1]
            )
            ...
            potential_dropoff_value = (
                final_halite - config.spawnCost - config.convertCost + delivered_halite
            )
        ...
        def make_plans():
            ...
            if (
                len(ships) == 1
                and len(shipyards) == 0
                and player_halite > config.spawnCost + config.convertCost
                and len(shipyards) == 0
            ):
            ...
            for i in reversed(range(len(unplanned))):
                if (
                    unplanned[i] in shipyards
                    and board.ships[unplanned[i]]["halite"] > 0
                ):
            ...
            while len(unplanned) > 0:
                ...
                if debug:
                    print(
                        ...
                        "value:",
                        get_current_value(
                            best_ship_path, plans, board.ships[ship]["halite"]
                        ),
                    )
        ...
        try:
            ...
            for shipyard in shipyards:
                if (
                    config.spawnCost <= player_halite
                    and config.episodeSteps - obs.step >= min_turns_to_spawn
                    and not shipyard in updated_ships
                    and ship_count < max_ships
                    and (
                        ship_count == 0
                        or remaining_halite / (ship_count + 1)
                        > config.spawnCost * spawn_payoff_factor
                    )
                ):
                    ship_count += 1
